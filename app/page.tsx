...
export default function Home() {
  ...
  useEffect(() => {
    const fetchInterpretations = async () => {
      ...
      try {
        ...
        setError("Failed to load interpretations. Plese try reloading the page");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      setError("Failed to delete interpretation. Plese try again.");
    }
  };
    ...
    <div>
      ...
      <div className="flex gap-4 mt-4 justify-end">
        <Link className="bg-slate-200 px-4 py-2 rounded-md uppercase text-sm
         ...
         href={`/edit/${interpretation.$id}`}>Edit
         </Link>
